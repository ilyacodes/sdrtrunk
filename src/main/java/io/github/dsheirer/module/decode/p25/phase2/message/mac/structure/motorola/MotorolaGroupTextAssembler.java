...
/**
 * Reassembles a P25 Motorola group text from a Header and Data Blocks.
 *
 * This is used for traffic channels and monitors:
 * - Phase 2: mac messaging
 */
public class MotorolaGroupTextAssembler
{
    ...
    private Map<Integer, MotorolaGroupTextDataBlock> mDataBlocks = new HashMap<>();
     ...
     */
    public MotorolaGroupTextAssembler(int timeslot)
    {
        mTimeslot = timeslot;
    }
     ...
     * @param mac to add
     * @return true if we can (now) assemble a complete group text from the header and data blocks.
     */
    public boolean add(MacStructure mac, long timestamp)
    {
        ...
        else if(mac instanceof MotorolaGroupTextDataBlock block)
        {
            mMostRecentTimestamp = timestamp;
    ...
    /**
     * Indicates if the assembler has a header and the correct number of data blocks to reassemble a group text.
     */
    ...
    /**
     * Assembles the group text once the header and data blocks have been collected.
     *
     * Note: do not invoke unless the add() methods indicate that the assembler is complete.
     * @return assembled group text
     * @throws IllegalStateException if the assembler can't assemble the group text.
     */
    public MotorolaGroupTextComplete assemble() throws IllegalStateException
    {
        if(!isComplete())
        {
            throw new IllegalStateException("Can't assemble group text - missing header or data block(s)");
        }
        ...
        for(int x = 1; x <= dataBlockCount; x++)
        {
            MotorolaGroupTextDataBlock block = mDataBlocks.get(x);
            reassembled.load(offset, block.getFragment());
            offset += DATA_BLOCK_FRAGMENT_LENGTH;
        }

        MotorolaGroupTextComplete complete = new MotorolaGroupTextComplete(reassembled, mHeader.getTalkgroup(),
